...
export function Navigation() {
  const [current, setCurrent] = useState<number | null>(5);
  ...
  return (
    <nav className="width-full flex justify-between items-center bg-white text-black dark:text-black shadow:b-lg relative">
      ...
      <ul className="flex gap-6 font-bold mr-28 text-lg navigation">
        ...
        <li
          className={`hover:text-orange-600 cursor-pointer ${current == 4 && `text-orange-600`}`}
        ...
        <li
          className={`hover:text-orange-600 cursor-pointer ${current == 5 && `text-orange-600`}`}
        >
          <User />
        </li>
      </ul>
